...
- [ ] spec 明确三类 tool 的职责边界
- [ ] spec 明确这些 tool 不提供写入能力
...
- [ ] spec 明确 `logical_path != local filesystem path`
- [ ] prompt contract 明确禁止把共享 artifact 逻辑路径映射为 `/workspace`
...
- [ ] backend 有可复用的 list/read/search service contract
- [ ] 文本与二进制 artifact 的返回语义清晰区分
...
- [ ] executor 能通过稳定接口获取 artifact list/read/search 结果
- [ ] 授权继续基于 channel membership 和当前运行时上下文
...
- [ ] 对大文件有明确截断或 metadata-only 规则
- [ ] 对 not found / forbidden / unsupported preview 有稳定错误语义
...
- [ ] channel 场景下 agent 能拿到 artifact runtime tools
- [ ] 非 channel 场景不会无谓注入这组 tool
...
- [ ] tool scope 自动继承当前 channel execution context
- [ ] 模型不需要自己声明或猜测 server/channel 身份
...
- [ ] 有测试覆盖 list/read/search 基本行为
- [ ] 有测试覆盖 channel trigger 后 agent 可使用 artifact tools 的运行时契约
...
- [ ] prompt 不再暗示 artifact 可直接通过本地文件 IO 访问
- [ ] prompt 明确共享材料应通过 runtime tools 读取
...
- [ ] 有用例覆盖跨 agent 共享材料读取
- [ ] 失败日志中不再出现把 logical path 误当本地路径的默认行为
...
- [ ] 非成员无法读取 channel artifacts
- [ ] 私有状态和 local mount 仍不在 tool 暴露范围内
