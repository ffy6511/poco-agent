...
- [ ] `ServerConversationMessage` 包含 `reactions`
- [ ] API 方法不暴露后端 snake_case 给 UI 层
...
- [ ] 主频道消息可以添加/取消 reaction
- [ ] thread drawer 内消息可以添加/取消 reaction
- [ ] search/inbox/saved 紧凑消息至少能显示 reaction groups
- [ ] 消息 action toolbar 有添加表情 icon button，点击后 picker 悬浮在按钮组下方
- [ ] picker 支持外部点击、Escape、选择 emoji 后关闭
- [ ] 所有新增用户可见文案走 i18n
...
- [ ] reaction 点击后 UI 立即反馈
- [ ] API 失败会回滚并提示
- [ ] 主列表和 thread drawer 的同一消息 reaction 状态保持一致
...
- [ ] 新增后端定向测试通过
- [ ] 现有 server channel message 测试不回归
...
- [ ] executor channel runtime reaction tool 注入测试通过
- [ ] executor-manager reaction proxy 测试通过
...
- [ ] 频道消息、thread drawer、search/inbox 紧凑消息没有布局溢出
- [ ] reaction 文案在中英文 locale 下都有值
...
- [ ] `cd backend && uv run -m alembic upgrade head` 通过
- [ ] migration downgrade 不遗留表或索引
